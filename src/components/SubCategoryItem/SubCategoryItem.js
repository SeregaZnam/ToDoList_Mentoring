import React, { Component } from 'react';
import './SubCategoryItem.css';

class SubCategoryItem extends Component {
  render() {
  	let subCategoryNode;

  	if (this.props.flagChangeTextSubCategory) {
  		subCategoryNode = <form
  			className="subcategory-item__form" 
            onSubmit={this.props.submitSubCategoryInput.bind(this)}
            >
            <input 
                type="text" 
                className="subcategory-item__form_input" 
                value={this.props.text}
                data-categoryindex={this.props.categoryIndex}
                data-subindex={this.props.subCategoryIndex}
                onChange={this.props.changeInputSubCategoryItem.bind(this)}
            />
  		</form>;
  	} else {
  		let idLabel = "subcategory-item__checkbox--label-" + this.props.categoryIndex + "-" + this.props.subCategoryIndex;

  		subCategoryNode = <span>
  			<input 
    			type="checkbox"
    			id={idLabel}
    			className="subcategory-item__checkbox"
    			checked={this.props.checkedSubCategory}
    			onChange={this.props.toggleShowSubTasks.bind(null, this.props.categoryIndex, this.props.subCategoryIndex)}
			/>
			<label htmlFor={idLabel} className="subcategory-item__text">{this.props.text}</label>
		</span>;
  	}

    return (
    	<div className="subsubcategory-item" >
   			<div className="subcategory-item">
	    		<div className="subcategory-item__left">
	    			{subCategoryNode}
	    			<input className="subcategory-item__editing" type="image" onClick={this.props.changeSubCategoryText.bind(null, this.props.categoryIndex, this.props.subCategoryIndex)} src="data:image/png;base64," />
	    		</div>
	    		<div className="subcategory-item__right">
	    			<input className="subcategory-item__delete" type="image" onClick={this.props.deleteSubCategoryItem.bind(null, this.props.categoryIndex, this.props.subCategoryIndex)} src="data:image/png;base64,"/>
	    			<input className="subcategory-item__add" type="image" src="data:image/png;base64," />
	    		</div>
	    	</div>
    	</div>
    );
  }
}

export default SubCategoryItem;
